...
&& npm i -f 
&& cd src 
&& echo "init database, it will take a long time" 
&& ts-node init.ts 
&& cd .. 
&& npm run build 
&& cd .. 
...
&& echo "start main" 
&& cd vite-ysdn 
&& pm2 start yarn --name main -- serve 
&& cd .. 
&& echo "start serve" 
&& cd server 
...
&& echo "use Command: [pm2 list] to list all your app" 
&& echo "use Command: [pm2 stop all] to stop them"
&& echo "use Command: [pm2 log] to see app log"
